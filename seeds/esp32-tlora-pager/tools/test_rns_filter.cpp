 ...
 * Ed25519 per announce, drop too much and the node silently stops hearing
 * traffic it needs. So the whole truth table is pinned here.
 *
 * The two constants come from rns/pktfilter.h, which mirrors
...
static bool keep(uint8_t type, uint8_t context) {
    bool first = rns_filter_keep_packet(type, context);
    bool second = rns_filter_keep_packet(type, context);
    assert(first == second);
    return first;
}
...
int main() {
    ...
    /* 2. An announce carrying a path response is kept. It is the answer to a
     * path request we sent, so it is the one announce a leaf pays for. */
    ...
    /* 5. The announce row in full: PATH_RESPONSE and nothing else survives.
     * This is the assertion that would catch an off-by-one in the constant
     * (RESPONSE 0x0A and CHANNEL 0x0E are its nearest neighbours). */
    for (unsigned c = 0; c < 256; c++) {
        bool expected = (c == CTX_PATH_RESPONSE);
        assert(keep(PKT_ANNOUNCE, (uint8_t)c) == expected);
    }
    assert(!keep(PKT_ANNOUNCE, CTX_RESOURCE));
    assert(!keep(PKT_ANNOUNCE, CTX_RESPONSE));
    assert(!keep(PKT_ANNOUNCE, CTX_CHANNEL));
    assert(!keep(PKT_ANNOUNCE, CTX_KEEPALIVE));
