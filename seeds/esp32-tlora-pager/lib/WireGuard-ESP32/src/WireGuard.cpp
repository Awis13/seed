...
#include "lwip/tcpip.h"  /* LOCK_TCPIP_CORE — required on ESP-IDF 5 / Arduino 3 */
...
/* IDF5 asserts if netif_* is called without TCPIP core lock. */
...
bool WireGuard::begin(const IPAddress& localIP, const char* privateKey, const char* remotePeerAddress, const char* remotePeerPublicKey, uint16_t remotePeerPort) {
	...
	// Platform init is independent of netif (RNG etc.).
	wireguard_platform_init();

	// ESP-IDF 5: netif_add / set_up / set_default require TCPIP core lock.
	LOCK_TCPIP_CORE();
