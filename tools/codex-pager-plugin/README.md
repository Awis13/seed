...
- `CODEX_PAGER_APP_SERVER_SOCKET` (default `~/.codex/ipc/ipc.sock`; the
  desktop application's live App Server socket). The bridge uses
  `codex app-server proxy` so pager turns are broadcast into the open Codex UI;
  it does not start a second hidden App Server.
